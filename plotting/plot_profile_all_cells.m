function plot_profile_all_cells(results_cell, field_name, profile_type, goodcells_saved, colors, condition_names, save_dir, savename, x_label, y_label)
% define bins
...
for i = 1:nconditions
    for j = 1:numel(goodcells_saved{i})
        plot(mid, sig_frac_cell{i}(j, :), ".-", "MarkerSize", 6, 'Color',  colors(i, :), 'LineWidth', .5)
        hold on;
    end
end
...
saveas(gca, [save_dir, field_name, savename, '_all_cells'], 'fig')
saveas(gca, [save_dir, field_name, savename, '_all_cells'], 'tif')
close
end